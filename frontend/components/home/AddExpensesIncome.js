  ...
  const submitExpenseIncome = async () => {
    try {
      ...
      const { data } = await axios.post(
        `${API}/${isExpense ? 'discharge' : 'income'}`,
        {
      ...
      Alert.alert(
        'Error',
        `Hubo un error al guardar ${isExpense ? 'Gasto.' : 'Ingreso.'}`,
        [{ text: 'Aceptar' }]
      )
      setLoading(false)
    }
      ...
      <ScrollView>
        ...
        >
          <View style={{ ...styles.row, alignItems: 'center', marginTop: 10 }}>
            <Text
              style={{ fontSize: 18, fontWeight: 'bold', letterSpacing: 0.3 }}
            >
              Cantidad
            </Text>
          </View>
          <View
            style={{
              ...
              borderColor: isExpense === 1 ? colors.redLight : colors.primary,
            }}
          >
            <FontAwesome
              style={{ marginLeft: 10 }}
              name='dollar'
              size={24}
              color={isExpense === 1 ? colors.redLight : colors.primary}
            />
            <TextInput
                ...
                renderItem={(itemData) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => selectCategory(itemData.item)}
                    >
